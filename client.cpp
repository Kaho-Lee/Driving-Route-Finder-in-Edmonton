...
void setup() {
    ...
    real_coordinate = (LonLat32*) malloc(max_path_len * sizeof(LonLat32));
    // no current path
    cur_path_len = 0;
    ...
    if (update_display_window) {
        ...
        if ( cur_path_len > 1 ) {
            ...
            */
            ...
            // dprintf("Max %d real_coordinate", max_path_len);
            // real_coordinate = (LonLat32*) malloc(max_path_len * sizeof(LonLat32));
            for (uint32_t i=0; i<cur_path_len; ++i){
              real_coordinate[i].lat = latitude_to_y(current_map_num, waypoints[i].lat);
              real_coordinate[i].lon = longitude_to_x(current_map_num, waypoints[i].lon);
              // dprintf("real coordinate of the waypoints %d x, y: %ld %ld",
              //     i, real_coordinate[i].lon, real_coordinate[i].lat);
            }
            for (uint32_t i = 0; i < cur_path_len-1;i++){
              // dprintf("drawingLine");
              uint16_t x1;
              uint16_t y1;
              uint16_t x2;
              uint16_t y2;
              x1 = real_coordinate[i].lon - screen_map_x;
              y1 = real_coordinate[i].lat - screen_map_y;
              x2 = real_coordinate[i+1].lon - screen_map_x;
              y2 = real_coordinate[i+1].lat - screen_map_y;
              // cursor_map_x = real_coordinate[i].lon;
              // cursor_map_y = real_coordinate[i].lat;
              // get_cursor_screen_x_y(x1, y1);
              // cursor_map_x = real_coordinate[i+1].lon;
              // cursor_map_y = real_coordinate[i+1].lat;
              // get_cursor_screen_x_y(x2, y2);
              // tft.fillRect(0, 20, 128, 12, ST7735_BLACK);
              //tft.fillCircle(1083, 1099, 20,ST7735_BLACK);
              tft.drawLine(x1, y1, x2, y2,ST7735_BLACK);
            }
            // free(real_coordinate);
